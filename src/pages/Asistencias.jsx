...
export default function Asistencias() {
  ...
  const { sincronizando, sincronizarAsignacionesDelDia, restaurarAsignacion } =
  ...
  useEffect(() => {
    const cargarDatos = async () => {
      ...
      try {
        ...
        console.log(
          '🔍 Estados encontrados:',
          todosEventosFecha?.map(e => e.estado) || []
        );
        ...
        if (eventosParaMostrar.length === 0) {
          ...
          const { data: eventosSoloCancelados, error: errorSoloCancelados } =
            await supabase
              .from('eventos_clase')
              .select('id, estado')
              .eq('fecha', fecha)
          ...
          if (
            !errorSoloCancelados &&
            Array.isArray(eventosSoloCancelados) &&
            eventosSoloCancelados.length > 0
          ) {
            console.log(
              `ℹ️ El ${fecha} hay ${eventosSoloCancelados.length} eventos pero están cancelados/eliminados.`
            );
          }

          console.log(
            '⚠️ No hay eventos para el día seleccionado, buscando eventos próximos (solo sugerencia)...'
          );
          const hoy = new Date();
          const proximosNDias = new Date();
          proximosNDias.setDate(hoy.getDate() + 30); // ampliar a 30 días

          const { data: eventosProximosData, error: eventosProximosError } =
            await supabase
              .from('eventos_clase')
              .select(
                `
      ...
      {clases.length === 0 ? (
        <div className='bg-white dark:bg-dark-surface p-12 rounded-2xl shadow-lg border border-gray-200 dark:border-dark-border text-center'>
          ...
          <div className='bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg'>
            <p className='text-blue-700 dark:text-blue-300 text-sm'>
              💡 <strong>Tip:</strong> Puedes cambiar la fecha usando el
              selector de arriba para ver clases de otros días.
            </p>
          </div>
        </div>
      ) : (
