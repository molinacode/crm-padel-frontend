    ...
    const cargarClasesExternas = async () => {
        ...
        `)
            ...
            const clasesProcesadas = clases.map(clase => ({
                ...clase,
                alumnosAsignados: clase.alumnos_clases?.map(ac => ac.alumnos).filter(Boolean) || [],
                            ...
                            <div className="flex justify-between items-start">
                                <div className="flex-1">
                                    ...
                                    <div className="text-sm text-gray-600 dark:text-dark-text2 space-y-1">
                                        <p>📅 {clase.dia_semana} • 🕐 {clase.hora_inicio} - {clase.hora_fin}</p>
                                        <p>📚 {clase.nivel_clase}</p>
                                        <p>👥 {clase.totalAlumnos} alumno{clase.totalAlumnos !== 1 ? 's' : ''} asignado{clase.totalAlumnos !== 1 ? 's' : ''}</p>
                                    </div>
                                    {clase.alumnosAsignados.length > 0 && (
                                        <div className="mt-2">
                                            <p className="text-xs text-gray-500 dark:text-dark-text2 mb-1">Alumnos:</p>
