...
interface PagosEditFormState {
  ...
  fecha_inicio: string;
  fecha_fin: string;
  clases_cubiertas: string;
  metodo: string;
  fecha_pago: string;
}

interface PagoEditarFuente {
...
interface PagoActualizarPayload {
  alumno_id: string;
  cantidad: number;
  tipo_pago: string;
  mes_cubierto: string | null;
  fecha_inicio: string | null;
  fecha_fin: string | null;
  clases_cubiertas: number | null;
  metodo: string;
  fecha_pago: string;
}

interface PagosEditarProps {
  onClose: () => void;
  onSuccess: (data: PagoActualizarPayload) => Promise<void>;
  pagoEditar?: PagoEditarFuente | null;
...
export default function PagosEditar({
  ...
  const [formData, setFormData] = useState<PagosEditFormState>({
    alumno_id: '',
    cantidad: '',
    tipo_pago: 'mensual',
    mes_cubierto: '',
  ...
  useEffect(() => {
    ...
    return scheduleEffectWork(() => {
      ...
      setFormData({
        ...
        cantidad:
          pagoEditar.cantidad !== undefined && pagoEditar.cantidad !== null
            ? String(pagoEditar.cantidad)
            : '',
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const pagoData: PagoActualizarPayload = {
        ...
        clases_cubiertas:
          formData.tipo_pago === 'clases'
            ? parseInt(formData.clases_cubiertas, 10)
            : null,
