...
interface EjercicioLike {
  nombre: string;
  tipo?: string | null;
  categoria?: string | null;
  dificultad?: 'Fácil' | 'Intermedio' | string | null;
  duracion_minutos?: number | null;
  description?: string | null;
}

interface MobileEjercicioCardProps {
  ejercicio: EjercicioLike;
  onActionClick?: () => void;
}
...
export default function MobileEjercicioCard({
  ejercicio,
  onActionClick,
}: MobileEjercicioCardProps) {
  const badges = useMemo(() => {
    const badgesArray = [
      { label: ejercicio.categoria || 'General', colorClass: 'bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300' },
      {
