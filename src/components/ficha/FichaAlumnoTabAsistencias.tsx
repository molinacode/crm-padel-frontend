interface ClaseNombre {
  nombre?: string | null;
}

interface AsistenciaFichaRow {
  id?: string;
  fecha?: string | null;
  estado?: string | null;
  esRecuperacion?: boolean;
  clases?: ClaseNombre | null;
  recuperacion?: { fecha_falta: string | null } | null;
}

interface FichaAlumnoTabAsistenciasProps {
  asistencias: AsistenciaFichaRow[];
}

export default function FichaAlumnoTabAsistencias({
  asistencias,
}: FichaAlumnoTabAsistenciasProps) {
  const asistenciasArray = asistencias;
                  ...
                  {asistencia?.esRecuperacion && asistencia?.recuperacion && (
                    ...
                    >
                      ...
                      <span className='text-[10px] opacity-75'>
                        (Falta:{' '}
                        {new Date(
                          asistencia.recuperacion.fecha_falta || ''
