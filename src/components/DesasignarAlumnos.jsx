...
export default function DesasignarAlumnos({ onClose, onSuccess, evento }) {
    ...
    const cargarAlumnosAsignados = async () => {
        try {
            ...
            console.log('👥 Alumnos asignados encontrados:', evento.alumnosAsignados?.length || 0);
        } catch (error) {
    ...
    const toggleAlumno = (alumnoId) => {
        ...
        if (nuevoSeleccionados.has(alumnoId)) {
            ...
            if (alumnosAExceso > 0 && nuevoSeleccionados.size >= alumnosAExceso) {
                alert(`❌ Solo puedes seleccionar hasta ${alumnosAExceso} alumno${alumnosAExceso !== 1 ? 's' : ''} para desasignar (exceso).`);
                return;
            }
    ...
    const desasignarAlumnos = async () => {
        ...
        if (alumnosAExceso > 0 && alumnosSeleccionados.size > alumnosAExceso) {
            alert(`❌ No puedes desasignar más de ${alumnosAExceso} alumno${alumnosAExceso !== 1 ? 's' : ''} (exceso).`);
            return;
        }
        ...
        try {
            ...
            alert(`✅ Se han desasignado ${alumnosSeleccionados.size} alumno${alumnosSeleccionados.size !== 1 ? 's' : ''} correctamente.`);
                ...
                <div className="bg-gradient-to-r from-red-50 to-orange-50 dark:from-red-900/30 dark:to-orange-900/30 p-6 border-b border-gray-200 dark:border-dark-border">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h2 className="text-2xl font-bold text-gray-900 dark:text-dark-text">Desasignar Alumnos</h2>
                                <p className="text-gray-600 dark:text-dark-text2">
                                    {evento.nombre} - {evento.alumnosPresentes > evento.maxAlumnos ? 'Clase sobresaturada' : 'Clase'} ({evento.alumnosPresentes}/{evento.maxAlumnos})
                                </p>
                            </div>
                        </div>
                        <button
                ...
                <div className="p-6 border-b border-gray-200 dark:border-dark-border">
                    <div className="bg-red-50 dark:bg-red-900/20 p-4 rounded-lg border border-red-200 dark:border-red-800/30">
                        <div className="grid grid-cols-2 gap-4 text-sm mb-4">
                            <div>
                                <p className="font-medium text-red-800 dark:text-red-200">Clase:</p>
                                <p className="text-red-900 dark:text-red-100">{evento.nombre}</p>
                            </div>
                            <div>
                            ...
                            <div>
                                <p className="font-medium text-red-800 dark:text-red-200">Estado actual:</p>
                                <p className="text-red-900 dark:text-red-100">
                                    {evento.alumnosPresentes}/{evento.maxAlumnos} alumnos{evento.alumnosPresentes > evento.maxAlumnos ? ' (Sobresaturada)' : ''}
                                </p>
                            </div>
                            <div>
                                <p className="font-medium text-red-800 dark:text-red-200">Alumnos a desasignar:</p>
                                <p className="text-red-900 dark:text-red-100">
                                    {alumnosAExceso} alumno{alumnosAExceso !== 1 ? 's' : ''} (mínimo)
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="flex justify-end gap-3 pt-3 border-t border-red-200 dark:border-red-800/30">
                            ...
                            >
                                {procesando ? 'Procesando...' : `Desasignar ${alumnosSeleccionados.size} alumno${alumnosSeleccionados.size !== 1 ? 's' : ''}`}
                            </button>
                        </div>
                    </div>
                </div>
                ...
                <div className="p-6 overflow-y-auto max-h-[calc(90vh-300px)]">
                    ...
                    {alumnosFiltrados.length === 0 ? (
                        <div className="text-center py-12">
                            ...
                            <h3 className="text-xl font-semibold text-gray-900 dark:text-dark-text mb-2">No hay alumnos asignados</h3>
