...
interface HistorialClasesProfesorProps {
  profesor: string;
}
...
interface HistorialItem {
  ...
  profesor: string;
  clases: ClaseInfo | null;
  ejercicios: EjercicioAsignado[];
}

export default function HistorialClasesProfesor({
  profesor,
}: HistorialClasesProfesorProps) {
  ...
  const cargarHistorial = async () => {
    try {
      ...
      const historialConEjercicios: HistorialItem[] = await Promise.all(
        (tematicasData || []).map(async tematica => {
          ...
          return {
            ...tematica,
            ejercicios: (ejerciciosData || []) as EjercicioAsignado[],
          };
        })
      );
  ...
  const historialFiltrado = historial.filter(item => {
    const matchesTematica =
      !filtroTematica || item.tematica.toLowerCase().includes(filtroTematica.toLowerCase());
    const matchesFecha = !filtroFecha || item.fecha_asignacion.includes(filtroFecha);
              ...
              <div className='bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 p-4 sm:p-6 border-b border-gray-200 dark:border-dark-border'>
                <div className='flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4'>
                  ...
                  <div className='text-right'>
                    <div className='text-sm text-gray-600 dark:text-dark-text2 mb-1'>Clase:</div>
                          ...
                          <div className='flex items-start justify-between mb-3'>
                            ...
                            <span
                              className={`px-2 py-1 text-xs rounded-full ${
                                ejercicio.categoria === 'Técnica'
                                  ? 'bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300'
                                  : ejercicio.categoria === 'Físico'
                                    ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300'
                                    : ejercicio.categoria === 'Táctico'
