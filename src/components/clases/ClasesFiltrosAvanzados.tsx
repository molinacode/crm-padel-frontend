...
export default function ClasesFiltrosAvanzados({
  ...
  return (
    <div className="bg-white dark:bg-dark-surface rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 overflow-hidden transition-all duration-300">
      <button
        onClick={() => setIsExpanded(!isExpanded)}
        className="w-full flex items-center justify-between p-4 hover:bg-gray-50 dark:hover:bg-dark-surface2 transition-colors duration-200"
      >
              ...
              <p className="text-xs text-blue-600 dark:text-blue-400 font-medium">
                {(() => {
                  const filtrosActivos = [
                    filtroNivel && 'Nivel',
                    filtroTipoClase && 'Tipo',
                    filtroFechaInicio && 'Fecha',
                  ].filter(Boolean);
                  return (
                    filtrosActivos.join(', ') +
        ...
        <div className="flex items-center gap-3">
          {tieneFiltros && !isExpanded && (
            <button
              onClick={e => {
                e.stopPropagation();
                limpiarFiltros();
              }}
              className="px-3 py-1 text-xs font-semibold text-red-600 dark:text-red-400 hover:text-red-700 dark:hover:text-red-300 bg-red-50 dark:bg-red-950/30 hover:bg-red-100 dark:hover:bg-red-950/50 rounded-lg transition-all duration-200 border border-red-200 dark:border-red-800 whitespace-nowrap"
