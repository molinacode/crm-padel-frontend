...
export default function DetalleClaseModal({
  ...
  const [tematica, setTematica] = useState<TematicaData | null>(null);
  const [ejercicios, setEjercicios] = useState<EjercicioClaseData[]>([]);
  ...
  const cargarDetalleClase = async () => {
    try {
      ...
      setClase((claseData as ClaseData | null) || null);
      ...
      const alumnosMapeados = ((alumnosData || []) as Array<{ alumnos: AlumnoData | null }>)
        .map(ac => ac.alumnos)
        .filter((a): a is AlumnoData => Boolean(a));
      ...
      if (tematicaError) {
        ...
        const tematicaRow = tematicaData[0] as TematicaData;
        setTematica(tematicaRow);
        ...
        const { data: ejerciciosData, error: ejerciciosError } = await supabase
          ...
          .eq('clase_id', claseId)
          .eq('tematica', tematicaRow.tematica);

        if (ejerciciosError) {
          console.error('Error cargando ejercicios:', ejerciciosError);
        } else {
          setEjercicios((ejerciciosData as EjercicioClaseData[] | null) || []);
        }
      }
    } catch (error) {
              ...
              {clase && (
                <div className='bg-gray-50 dark:bg-dark-surface2 rounded-xl p-4'>
                  ...
                  <div className='grid grid-cols-2 gap-4 text-sm'>
                    ...
                    <div><span className='text-gray-600 dark:text-dark-text2'>Nivel:</span><span className='ml-2 font-medium text-gray-900 dark:text-dark-text'>{clase.nivel_clase}</span></div>
                    <div><span className='text-gray-600 dark:text-dark-text2'>Tipo:</span><span className='ml-2 font-medium text-gray-900 dark:text-dark-text'>{clase.tipo_clase}</span></div>
                    <div><span className='text-gray-600 dark:text-dark-text2'>Profesor:</span><span className='ml-2 font-medium text-gray-900 dark:text-dark-text'>{clase.profesor || 'Sin asignar'}</span></div>
                    <div><span className='text-gray-600 dark:text-dark-text2'>Día:</span><span className='ml-2 font-medium text-gray-900 dark:text-dark-text'>{clase.dia_semana}</span></div>
                    <div><span className='text-gray-600 dark:text-dark-text2'>Horario:</span><span className='ml-2 font-medium text-gray-900 dark:text-dark-text'>{clase.hora_inicio} - {clase.hora_fin}</span></div>
                  </div>
                </div>
              )}
              ...
              {tematica && (
                <div>
                  ...
                  <div className='bg-blue-50 dark:bg-blue-900/20 rounded-xl p-4 border border-blue-200 dark:border-blue-800/30'>
                    ...
                    <div className='text-sm text-gray-600 dark:text-dark-text2'>
                      Asignada el:{' '}
                      {new Date(tematica.fecha_asignacion).toLocaleDateString('es-ES')}
                    </div>
                  </div>
                </div>
              )}
