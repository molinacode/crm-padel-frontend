  ...
  try {
    ...
    for (const tabla of tablas) {
      try {
        ...
        if (error) {
          resultados.errores.push(`❌ Error en tabla ${tabla}: ${error.message}`);
        ...
        resultados.errores.push(`❌ Excepción en tabla ${tabla}: ${err.message}`);
    ...
    try {
      ...
      if (error && error.code === '42703') {
        resultados.advertencias.push('⚠️ Campo "origen" no existe en alumnos_clases');
        console.warn('⚠️ Campo "origen" no existe en alumnos_clases');
      } else if (error) {
        resultados.errores.push(`❌ Error verificando campo origen: ${error.message}`);
      } else {
        resultados.campos.origen = { existe: true };
        console.log('✅ Campo "origen" existe en alumnos_clases');
      }
    } catch (err) {
      resultados.errores.push(`❌ Excepción verificando origen: ${err.message}`);
    }
    ...
    try {
      ...
      if (error && error.code === '42703') {
        resultados.advertencias.push('⚠️ Campos de modificación no existen en eventos_clase');
        console.warn('⚠️ Campos de modificación no existen en eventos_clase');
      } else if (error) {
        resultados.errores.push(`❌ Error verificando campos de modificación: ${error.message}`);
      } else {
        resultados.campos.modificacion = { existe: true };
        console.log('✅ Campos de modificación existen en eventos_clase');
      }
    } catch (err) {
      resultados.errores.push(`❌ Excepción verificando campos de modificación: ${err.message}`);
    }
    ...
    try {
      ...
      if (error) {
        resultados.errores.push(`❌ Error en consulta de eventos: ${error.message}`);
        console.error('❌ Error en consulta de eventos:', error);
      } else {
        console.log(`✅ Consulta de eventos exitosa: ${data?.length || 0} eventos`);
      }
    } catch (err) {
      resultados.errores.push(`❌ Excepción en consulta de eventos: ${err.message}`);
    }
    ...
    try {
      ...
      if (error) {
        resultados.errores.push(`❌ Error en consulta de asignaciones: ${error.message}`);
        console.error('❌ Error en consulta de asignaciones:', error);
      } else {
        console.log(`✅ Consulta de asignaciones exitosa: ${data?.length || 0} asignaciones`);
      }
    } catch (err) {
      resultados.errores.push(`❌ Excepción en consulta de asignaciones: ${err.message}`);
    }
    ...
    Object.entries(resultados.tablas).forEach(([tabla, info]) => {
      console.log(`   ${tabla}: ${info.existe ? '✅' : '❌'} (${info.registros} registros)`);
    });
