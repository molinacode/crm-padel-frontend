...
interface EventoLike {
  ...
  alumnosJustificados?: unknown[];
}

interface HuecosInfo {
  huecosReales: number;
 ...
 */
export function useHuecosDisponibles(
  evento: EventoLike | null,
  clase: Pick<Clase, 'tipo_clase' | 'nombre'> | null = null
) {
  const [huecosInfo, setHuecosInfo] = useState<HuecosInfo>({
    huecosReales: 0,
    alumnosPresentes: 0,
    alumnosAsignados: 0,
    alumnosLiberados: 0,
  ...
  const calcularHuecos = useCallback(async () => {
    ...
    try {
      ...
      if (!claseData) {
        ...
        claseData = (data as Pick<Clase, 'tipo_clase' | 'nombre'> | null) || null;
      }
      ...
      const resultado = calcularHuecosDesdeSupabase({
        ...
        justificadosData: ((evento.alumnosJustificados || []) as Array<{
          alumno_id?: string | null;
          id?: string | null;
        }>).map(item => ({
          alumno_id: item.alumno_id || undefined,
