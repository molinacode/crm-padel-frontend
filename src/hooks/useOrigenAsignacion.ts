 ...
 */
export function useOrigenAsignacion(
  claseId: string | null,
  claseActual: Clase | null,
  asignadosCount = 0
) {
  const [origenAsignacion, setOrigenAsignacion] =
    useState<OrigenAsignacion>('escuela');
      ...
      try {
        ...
        if (asignadosRes && asignadosRes.length > 0) {
          ...
          const origenes = asignadosRes
            .map(a => a.origen as OrigenAsignacion | null | undefined)
            .filter(o => o !== null && o !== undefined);

          if (origenes.length > 0) {
            const origenMasComun = obtenerOrigenMasComun(
              origenes
            ) as OrigenAsignacion;
  ...
  const actualizarOrigen = useCallback(
    async (nuevoOrigen: OrigenAsignacion) => {
      if (!claseId) {
