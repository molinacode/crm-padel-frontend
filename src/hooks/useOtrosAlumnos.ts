...
interface AlumnoInterno {
  ...
  clasesInternas: Array<{ id: string; nombre: string | null; tipo_clase: string | null }>;
}

export function useOtrosAlumnos() {
  const [alumnos, setAlumnos] = useState<AlumnoInterno[]>([]);
  ...
  const cargar = useCallback(async () => {
    try {
      ...
      const alumnosUnicos: Record<string, AlumnoInterno> = {};
      alumnosInternos.forEach((asignacion) => {
        const alumno = asignacion.alumnos as Omit<AlumnoInterno, 'clasesInternas'>;
        if (!alumnosUnicos[alumno.id]) {
          alumnosUnicos[alumno.id] = { ...alumno, clasesInternas: [] };
        }
        alumnosUnicos[alumno.id].clasesInternas.push(
  ...
  const eliminarAlumno = useCallback(async (alumnoId: string) => {
