...
export function useInternasMes() {
  const [items, setItems] = useState<InternaMesItem[]>([]);
  ...
  const cargar = useCallback(async () => {
    ...
    try {
      ...
      let clasesInternas: ClaseInterna[] = [];
      if (claseIds.length > 0) {
        ...
        clasesInternas = ((clasesData as ClaseInterna[] | null) || []).filter(
          c => {
      ...
      let pagosMapa = new Map<string, PagoInterna>();
      if (claseIds.length > 0) {
        ...
        pagosMapa = new Map(
          (((pagosInternas as PagoInterna[] | null) || []).map(p => [
            `${p.clase_id}|${p.fecha}`,
            p,
          ]))
        );
      ...
      const resultados = (eventos || [])
        .filter(ev => isNonEmptyString(ev.clase_id) && internasIdSet.has(ev.clase_id))
        .sort(
          (a, b) =>
        ...
        .map(ev => {
          ...
          const key = `${claseId}|${ev.fecha}`;
          const pago = pagosMapa.get(key);
          return {
            id: ev.id,
            eventoId: ev.id,
            claseId: ev.clase_id,
            nombre: nombrePorClase.get(claseId) || 'Clase interna',
            fecha: ev.fecha,
